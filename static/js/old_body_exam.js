$(function () {
    ...
    var btn_susp = toolbar.find('#susp').linkbutton({ iconCls: 'icon-save', plain: true });
    ...
    btn_susp.bind('click', function () {
        if ($(this).linkbutton('options').disabled == false) {
            form.form('submit', {
                url: '/old/body_exam_suspend/', method: 'POST',
                onSubmit: function (param) {
                    ...
                    if (form.find('input[name=electr_gram]').length && !form.find('input[name=electr_gram]').is(":checked")) {
                        $.messager.alert('提示', '请选择辅助检查-心电图', 'info');
                        return false;
                    }
                ...
                success: function (json_data) {
                    ...
                    if (data.success) {
                        panel.panel('refresh');
                        $.messager.alert('提示', '老年人健康体检暂存完成', 'info');
                    } else {
                        $.messager.alert('提示', '老年人健康体检暂存失败', 'info');
                    }
                }
            })
        }
    ...
    panel.panel({
        href: '/old/body_exam_form/', method: 'POST',
        onLoad: function () {
            if (form.find('input').length) {
                btn_save.linkbutton('enable');
                btn_susp.linkbutton('enable');
            } else {
                btn_save.linkbutton('disable');
                btn_susp.linkbutton('disable');
            }
        }
    });
});