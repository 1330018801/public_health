...
def user_update(request):
    ...
    if request.user.is_superuser or request.user.userprofile.role.name != u"卫生院管理员":
        ...
        if not town_clinic:
            ...
            users = User.objects.filter(Q(userprofile__clinic=town_clinic) | 
            Q(userprofile__clinic__town_clinic=town_clinic))
...
def is_date(var_obj):
    ...
    if len(var_obj) == 10:
        rule = '(([0-9]{3}[1-9]|[0-9]{2}[1-9][0-9]{1}|[0-9]{1}[1-9][0-9]{2}|[1-9][0-9]{3})-(((0[13578]|1[02])-(0[1-9]|
        [12][0-9]|3[01]))|((0[469]|11)-(0[1-9]|[12][0-9]|30))|(02-(0[1-9]|[1][0-9]|2[0-8]))))|((([0-9]{2})(0[48]|
    ...
    if request.method == 'POST':
        ...
        if userprofile.resident is None:
            if form.is_valid():
                ...
                #if IsDate(request.POST['birthday']) and IsString(request.POST['address']) and 
                IsString(request.POST['mobile']) and IsEmail(request.POST['email']):
...
def resident_service_list(request):
    try:
        ...
        #service_items_finished = resident.service_items_finished(datetime(datetime.today().year-1, 
        datetime.today().month, datetime.today().day, 0, 0, 0, 0,))
...
def town_staff_statistics_records(request):
    ...
    if not town_clinic:
        ...
        records = WorkRecord.objects.filter(Q(provider__userprofile__clinic=town_clinic) | 
...
def town_staff_statistics_resident(request):
    ...
    if not town_clinic:
        ...
        work_records = WorkRecord.objects.filter(Q(provider__userprofile__clinic=town_clinic) | 
        Q(provider__userprofile__clinic__town_clinic=town_clinic))
...
def sms_time_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            sms_time_1 = sms_time_1.filter(service_type=service_type, service_item=service_item, 
            service_time__year=service_time_year)
...
def workload_stat_datagrid(request):
    """
    函数说明：计算各个卫生院各个服务类别的工作量及合计
    """
    workload = collections.OrderedDict()
    for town_clinic in Clinic.in_town.all():
        workload[town_clinic.name] = dict()
        for service_type in Service.types.all():
...
def payment_stat_datagrid(request):
    """
    函数说明：计算各个卫生院各个服务类别的工作量及合计
    """
    ...
    for record in WorkRecord.objects.filter(status=WorkRecord.FINISHED, submit_time__gte=new_year_day()):
        try:
            ...
            if record.service_item and record.service_item.is_service_item:  # 这是一个计费项目
                service_type = record.service_item.service_type
                workload[town_clinic.name][service_type.alias] += record.service_item.price
                workload['合计'][service_type.alias] += record.service_item.price
